...
function collectLoadedStaticAssets(): string[] {
  ...
  return [...scriptUrls, ...linkUrls].filter((url) =>
...
function requestOfflineCacheWarm(
...
async function hasRequiredOfflineCache(): Promise<boolean> {
  ...
  const results = await Promise.all(
    REQUIRED_OFFLINE_ROUTES.map((route) => isRouteCached(route)),
  );
    ...
    async function refreshPendingCount() {
      try {
        const count = await db.syncQueue
          .where("status")
          .anyOf(["pending", "failed", "syncing", "blocked"])
          .count();
    ...
    if (!("serviceWorker" in navigator)) {
      ...
      const cleanupDevWorker = async () => {
        try {
          ...
          if (
            (wasControlled || removedWorker || deletedCache) &&
            !alreadyReloaded
          ) {
