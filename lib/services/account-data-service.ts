...
import type { AuthCacheEntry, Bill, BillItem, Customer, CustomerPayment, Product, Settings, Shift, StockMovement, SyncConflict, SyncQueueItem } from '@/types/domain';
...
async function clearRuntimeDb(): Promise<void> { await Promise.all([db.products.clear(),db.bills.clear(),db.billItems.clear(),db.stockMovements.clear(),db.customerPayments.clear(),db.customers.clear(),db.shifts.clear(),db.settings.clear(),db.authCache.clear(),db.syncQueue.clear(),db.syncConflicts.clear().catch(()=>undefined)]); }
async function buildSnapshot(uid: string): Promise<AccountSnapshot> {
  const [products,bills,billItems,stockMovements,customerPayments,customers,shifts,settings,authCache,syncQueue,syncConflicts] = await Promise.all([db.products.toArray(),db.bills.toArray(),db.billItems.toArray(),db.stockMovements.toArray(),db.customerPayments.toArray(),db.customers.toArray(),db.shifts.toArray(),db.settings.toArray(),db.authCache.toArray(),db.syncQueue.toArray(),db.syncConflicts.toArray().catch(()=>[] as SyncConflict[])]);
  return { uid, deviceId: getOrCreateDeviceId(), savedAt: new Date().toISOString(), summary: await getLocalDataSummary(), products,bills,billItems,stockMovements,customerPayments,customers,shifts,settings,authCache,syncQueue,syncConflicts };
}
