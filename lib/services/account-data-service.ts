...
import type { AuthCacheEntry, Bill, BillItem, Customer, CustomerPayment, Product, Settings, StockMovement, SyncConflict, SyncQueueItem } from '@/types/domain';
...
async function clearRuntimeDb(): Promise<void> { await Promise.all([db.products.clear(),db.bills.clear(),db.billItems.clear(),db.stockMovements.clear(),db.customerPayments.clear(),db.customers.clear(),db.settings.clear(),db.authCache.clear(),db.syncQueue.clear(),db.syncConflicts.clear().catch(()=>undefined)]); }
async function buildSnapshot(uid: string): Promise<AccountSnapshot> {
  const [products,bills,billItems,stockMovements,customerPayments,customers,settings,authCache,syncQueue,syncConflicts] = await Promise.all([db.products.toArray(),db.bills.toArray(),db.billItems.toArray(),db.stockMovements.toArray(),db.customerPayments.toArray(),db.customers.toArray(),db.settings.toArray(),db.authCache.toArray(),db.syncQueue.toArray(),db.syncConflicts.toArray().catch(()=>[] as SyncConflict[])]);
  return { uid, deviceId: getOrCreateDeviceId(), savedAt: new Date().toISOString(), summary: await getLocalDataSummary(), products,bills,billItems,stockMovements,customerPayments,customers,settings,authCache,syncQueue,syncConflicts };
}
...
export async function restoreAccountSnapshot(uid: string): Promise<boolean> {
  ...
  await db.transaction('rw', [db.products,db.bills,db.billItems,db.stockMovements,db.customerPayments,db.customers,db.settings,db.authCache,db.syncQueue,db.syncConflicts], async()=>{
    if(snapshot.products.length) await db.products.bulkPut(snapshot.products); if(snapshot.bills.length) await db.bills.bulkPut(snapshot.bills); if(snapshot.billItems.length) await db.billItems.bulkPut(snapshot.billItems); if(snapshot.stockMovements.length) await db.stockMovements.bulkPut(snapshot.stockMovements); if(snapshot.customerPayments.length) await db.customerPayments.bulkPut(snapshot.customerPayments); if(snapshot.customers?.length) await db.customers.bulkPut(snapshot.customers); if(snapshot.settings.length) await db.settings.bulkPut(snapshot.settings); if(snapshot.authCache.length) await db.authCache.bulkPut(snapshot.authCache); if(snapshot.syncQueue.length) await db.syncQueue.bulkPut(snapshot.syncQueue); if(snapshot.syncConflicts.length) await db.syncConflicts.bulkPut(snapshot.syncConflicts);
  });
  return true;
}
